# John Rigoni
# This file is the implimentation of particle swarm optimization for regression
...
# The class Swarm hold the population of Particles. It allows for the swarm to update. 
# It also allows for the testing of the test dataset.
...
class Particle():
    ...
    def evaluate(self):
        ...
        self.cur_err = self.network.out_layer.final_loss
        # save best
...
class Swarm():
    ...
    #   C1 & C2 : parameter for cognitive and social componenets
    #   omega : parameter for inertia component
    def __init__(self, Xtrain, C1, C2, omega, particle_count):
    ...
    def minimize(self, iterations):
        for _ in range(iterations):
            ...
            for i in range(self.particle_count):
                ...
                if self.particle_list[i].network.out_layer.final_loss < self.global_best_total:
                    self.global_best_index = i
                    self.global_best_total =  self.particle_list[i].network.out_layer.final_loss
    ...
    def test(self, Xtest):
        ...
        p = self.particle_list[self.global_best_index].network.out_layer.final_loss
        print(p)
        # save loss
        self.tot_loss = self.particle_list[self.global_best_index].network.out_layer.final_loss
        pass
...
class Network():
    def __init__(self, Xtrain):
        ...
        self.network = RegressionNetwork(inputs= Xtrain, momentum =0.5)
...
if __name__ == "__main__":
    ...
    # X = data_sets.abalone_parse.get_data()
    # X = data_sets.machine_parse.get_data()
    X = data_sets.forestfires_parse.get_data()
    ...
    for fold in range(10):
        ...
        swarm = Swarm(Xtrain, C1=0.5, C2=1, omega=0.9, particle_count=20 )
        # perform minimization for given amount of iterations
        swarm.minimize(iterations=200)
        # calculate the loss on the test set
        swarm.test(Xtest)
        # add loss to list
        total_cor_per.append( swarm.tot_loss )
   
    # calculate mean and stdev, write to file, print
    meaned = mean(total_cor_per)
    std_deved = stdev(total_cor_per)
    # with open("data_out/forest_fires/swarm_out.txt", "a") as w:
