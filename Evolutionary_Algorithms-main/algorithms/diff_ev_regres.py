# John Rigoni
# This file is the implimentation of differential evolution for regression
...
# objects will instantiate one Network object each. 
# 
...
class Organism():
    ...
    # Input: Xtrain = training set in standard dataframe format
    ...
    def evaluate(self):
        ...
        self.netw_loss = self.network.out_layer.final_loss
...
class Nature():
    ...
    def evolve(self, iterations, beta):
        # loop for num of generations
        for _ in range(iterations):
            # the population list for the next generation
            self.to_pop_list = []
            ...
            for i in range(self.pop_size):
                ...
                # print("target loss: ", target_loss, "  vs_target loss: ",vs_target_loss)
    ...
    def best_test(self, Xtest):
        ...
        self.best_loss = self.pop_list[self.best_index].network.out_layer.final_loss
...
if __name__ == "__main__":
    ...
    for fold in range(10):
        ...
        nature = Nature(Xtrain, pop_size= 20, pr=0.1)
        # perform evolution for given amount of iterations
        nature.evolve(iterations=300, beta=1)
        # calculate the loss on the test set
        nature.best_test(Xtest)
        # add loss to list
        total_loss.append( nature.best_loss )
    # calculate mean and stdev, write to file, print
    meaned = mean(total_loss)
    std_deved = stdev(total_loss)
    # with open("data_out/forest_fires/diff_ev_out.txt", "a") as w:
